...
/*void UtfFile::ToggleEncryption(uint8_t *top)
{
	big_endian = false;
		...
		if ((col.flags & STORAGE_MASK) == STORAGE_CONSTANT)
		{
			switch (col.flags & TYPE_MASK)
			{
				...
				case TYPE_DATA:
				{
                    ...
                    //DPRINTF("Binary (position: %x, size: %x)\n", position, col.constant_data_size);
		...
		for (uint16_t i = 0; i < val16(table_hdr->num_columns); i++)
		{
			...
			switch (current_value.type)
			{
				...
				case TYPE_DATA:
				{
					...
					row_ptr += 8;
                    //uint32_t position = Utils::DifPointer(binary_data, table_hdr);
                    //DPRINTF("Binary (position: %x, size: %x): %s\n", position, current_value.data_size, columns[i].name.c_str());
				}
				break;
...
bool UtfFile::SetByte(const std::string &name, uint8_t byte, unsigned int row)
{
	if (row >= rows.size())
		return false;
...
bool UtfFile::SetWord(const std::string &name, uint16_t word, unsigned int row)
{
	if (row >= rows.size())
		return false;
...
bool UtfFile::SetDword(const std::string &name, uint32_t dword, unsigned int row)
{
	if (row >= rows.size())
		return false;
...
bool UtfFile::SetQword(const std::string &name, uint64_t qword, unsigned int row)
{
	if (row >= rows.size())
		return false;
...
bool UtfFile::SetFloat(const std::string &name, float f, unsigned int row)
{
	if (row >= rows.size())
		return false;
...
bool UtfFile::SetString(const std::string &name, const std::string &str, unsigned int row)
{
	if (row >= rows.size())
		return false;
...
bool UtfFile::SetBlob(const std::string &name, uint8_t *blob, unsigned int blob_size, unsigned int row, bool take_ownership)
{
	if (row >= rows.size())
		return false;
