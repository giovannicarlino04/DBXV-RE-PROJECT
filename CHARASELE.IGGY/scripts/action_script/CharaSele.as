package action_script
{
   ...
   public class CharaSele
   {
      ...
      public function CharaSele()
      {
         ...
         var _loc2_:int;
         super();
         this.m_callback = new action_script.Callback(ReceiveType_Num);
         this.m_timeline = null;
         this.m_timer = new action_script.CountDownTimer();
         ...
         while(CharacterMax > _loc11_)
         {
            ...
            else
            {
               ...
               while(0 <= _loc12_)
               {
                  ...
                  else if((_loc7_ = String((_loc6_ = _loc3_[_loc12_])[CharaList.VarTypeCode])) == CharaList.InvalidCode)
                  {
                     this.SetUserDataInt(ReceiveType_UnlockVarStart + _loc5_,0);
                  }
                  else
                  {
                     _loc8_ = int(_loc6_[CharaList.VarTypeMid]);
                     _loc2_ = true;
                     if(!this.m_callback.GetUserDataValidFlag(ReceiveType_UnlockVarStart + _loc5_))
                     {
                        _loc9_ = int(_loc6_[CharaList.VarTypeUnlockIndex]);
                        _loc10_ = _loc12_;
      ...
      private function startMain() : void
      {
         ...
         var _loc17_:int;
         if(_loc17_ = this.m_callback.GetUserDataInt(ReceiveType_FlagLocalBattle))
         {
         ...
         (_loc18_ = this.getReadyIconMc(0)).visible = true;
         _loc14_ = 0;
         ...
         while(this.m_chara_list_num > _loc14_)
         {
            ...
            {
               _loc7_ = String(_loc5_[0][CharaList.VarTypeCode]);
         ...
         (_loc19_ = this.m_timeline.cha_select_cur).icn_lock.visible = false;
         this.ResetIcons();
         _loc14_ = 0;
         ...
         {
            if(!(_loc13_ = this.getMcChamyset(_loc14_)))
            {
            }
            _loc14_++;
         }
         var _loc20_:int;
         if((_loc20_ = this.m_callback.GetUserDataInt(ReceiveType_Time)) <= 0)
         {
      ...
      private function main(param1:Event) : void
      {
         ...
         var _loc9_:int = 0;
         var _loc10_:int;
         if((_loc10_ = this.m_callback.GetUserDataInt(ReceiveType_Time)) > 0)
         {
            if(this.m_timer)
            {
         ...
         var _loc14_:MovieClip = (_loc13_ = (_loc12_ = this.m_timeline.cha_select)["chara_icn_set0" + (this.m_select_icon_column + 1)])["nest_charaselect0" + (this.m_select_icon_row + 1)];
         ...
         if(this.m_flag_decide)
         {
            if(this.m_timeline.cha_parameter.currentLabel == "start" || this.m_timeline.cha_parameter.currentLabel == "wait")
            {
               ...
               _loc3_ = int(this.m_select_info[this.m_current_player_index][SelectInfoTypeListIndex]);
               if(_loc4_ = this.getCharaInfo(_loc3_))
               {
         ...
         if(this.m_timeline.cha_parameter.currentLabel == "end_comp")
         {
            ...
            if(!(_loc16_ = this.m_callback.GetUserDataInt(ReceiveType_FlagLocalBattle)))
            {
               if(_loc10_ > 0)
               {
      ...
      public function GetSelectVarInfo(param1:int) : Array
      {
         ...
         var _loc2_:int = int(this.m_select_info[param1][SelectInfoTypeListIndex]);
         var _loc3_:Array = this.getCharaInfo(_loc2_);
         if(!_loc3_)
         {
            return null;
         }
         var _loc4_:int = (_loc4_ = (_loc4_ = int(this.m_select_info[param1][SelectInfoTypeVarIndex])) + _loc3_.length) % _loc3_.length;
      ...
      private function getReadyIconMc(param1:int) : MovieClip
      {
         ...
         return this.m_timeline.cha_parameter.nest_ready["btnact_rb0" + (_loc3_ + 1)];
      }
      ...
      private function changeLR(param1:Boolean) : void
      {
         ...
         do
         {
            ...
            {
               if(!(_loc4_ = this.m_callback.GetUserDataInt(ReceiveType_FlagSelectAvatar)))
               {
         ...
         do
         {
            if(param1)
            {
               ...
               if(this.m_select_row < 0)
               {
                  ...
                  --this.m_select_column;
                  this.m_select_column += this.m_chara_num_column;
                  this.m_select_column %= this.m_chara_num_column;
               }
            }
            else
      ...
      private function changeVar(param1:Boolean) : void
      {
         ...
         do
         {
            ...
            this.m_select_var += _loc4_.length;
            this.m_select_var %= _loc4_.length;
      ...
      private function changeCurrentPlayer(param1:Boolean) : void
      {
         ...
         do
         {
            ...
            switch(_loc3_)
            {
               case PlayerTeamTypeFri:
                  ...
                  if((_loc5_ = this.m_callback.GetUserDataInt(ReceiveType_PartyNpcNum)) > _loc4_)
                  {
                     _loc2_ = false;
                  }
         ...
         {
            _loc7_ = int(this.m_select_info[this.m_current_player_index][SelectInfoTypeListIndex]);
            this.m_select_row = this.calcIconIndexRow(_loc7_);
            this.m_select_column = this.calcIconIndexColumn(_loc7_);
            this.m_select_var = this.m_select_info[this.m_current_player_index][SelectInfoTypeVarIndex];
         }
      ...
      private function setSelectChara() : void
      {
         ...
         var _loc13_:MovieClip = (_loc12_ = (_loc11_ = this.m_timeline.cha_select)["chara_icn_set0" + (_loc10_ + 1)])["nest_charaselect0" + (_loc9_ + 1)];
         this.m_select_icon_row = _loc9_;
         this.m_select_icon_column = _loc10_;
         _loc1_ = 0;
         while(CharaVarIndexNum > _loc1_)
         {
            if(_loc4_ = this.getMcChamyset(_loc1_))
            {
      ...
      public function sendCharaInfo(param1:int) : void
      {
         ...
         var _loc4_:int = int(this.m_select_info[param1][SelectInfoTypeVarIndex]);
      ...
      private function updateCharaIcon() : void
      {
         ...
         while(IndexNumColumn + 1 > _loc2_)
         {
            ...
            {
               if(_loc4_ = _loc3_["nest_charaselect0" + (_loc1_ + 1)])
               {
                  ...
                  {
                     _loc4_.visible = true;
                     if(!(_loc6_ = this.m_callback.GetUserDataInt(ReceiveType_FlagSelectAvatar)) && this.m_current_player_index != PlayerIndexOwn && this.checkAvatar(_loc5_))
                     {
                        _loc4_.gotoAndPlay("grayout");
                     }
                     else if(this.m_select_icon_column == _loc2_ && this.m_select_icon_row == _loc1_)
                     {
                        (_loc7_ = this.m_timeline.cha_select_cur).visible = true;
      ...
      private function updateSkill() : void
      {
         ...
         if(this.m_flag_skill)
         {
            ...
            while(SkillMax > _loc7_)
            {
               ...
               _loc5_ = Number(_loc8_.inact_skill["skill0" + (_loc7_ + 1)].sys_skill.width);
            ...
            _loc2_ = Number(_loc8_.inact_skill.skill08.sys_skill.width);
