package action_script
{
   ...
   public class CharaSele
   {
      ...
      private var m_callback:action_script.Callback = null;
      
      private var m_timeline:MovieClip = null;
      
      private var m_timer:action_script.CountDownTimer = null;
      ...
      {
         super();
         m_callback = new action_script.Callback(ReceiveType_Num);
         m_timeline = null;
         m_timer = new action_script.CountDownTimer();
         ...
         m_dlc3_chara_name = [CharaList.InvalidCode,CharaList.InvalidCode,CharaList.InvalidCode];
         try
         {
      ...
      private function waitDlcInfo(param1:Event) : void
      {
         ...
         if(!m_callback.GetUserDataValidFlag(ReceiveType_FlagDlc1))
         {
            return;
         }
         if(!m_callback.GetUserDataValidFlag(ReceiveType_FlagDlc2))
         {
            return;
         }
         if(!m_callback.GetUserDataValidFlag(ReceiveType_FlagDlc3))
         {
            return;
         }
         if(!m_callback.GetUserDataValidFlag(ReceiveType_FlagFirstJa))
         {
            return;
         }
         if(!m_callback.GetUserDataValidFlag(ReceiveType_FlagFirstOther))
         {
            return;
         }
         var _loc3_:int = m_callback.GetUserDataInt(ReceiveType_FlagDlc1);
         var _loc4_:int = m_callback.GetUserDataInt(ReceiveType_FlagDlc2);
         var _loc5_:int = m_callback.GetUserDataInt(ReceiveType_FlagDlc3);
         var _loc6_:int = m_callback.GetUserDataInt(ReceiveType_FlagFirstJa);
         var _loc7_:int = m_callback.GetUserDataInt(ReceiveType_FlagFirstOther);
         m_chara_list = CharaList.GetList(_loc3_,_loc4_,_loc5_,_loc6_,_loc7_);
         _loc2_ = 0;
         while(m_chara_list.length > _loc2_)
         {
            if(m_chara_list[_loc2_][0][0] == CharaList.Dlc3CodeChara0)
            {
               m_chara_list[_loc2_][0][0] = m_dlc3_chara_name[0];
            }
            else if(m_chara_list[_loc2_][0][0] == CharaList.Dlc3CodeChara1)
            {
               m_chara_list[_loc2_][0][0] = m_dlc3_chara_name[1];
            }
            else if(m_chara_list[_loc2_][0][0] == CharaList.Dlc3CodeChara2)
            {
               m_chara_list[_loc2_][0][0] = m_dlc3_chara_name[2];
            }
            _loc2_++;
         }
         ...
         while(CharacterMax > _loc2_)
         {
            ...
            else
            {
               ...
               while(0 <= _loc3_)
               {
                  ...
                  else if((_loc9_ = String((_loc8_ = _loc5_[_loc3_])[CharaList.VarTypeCode])) == CharaList.InvalidCode)
                  {
                     SetUserDataInt(ReceiveType_UnlockVarStart + _loc7_,0);
                  }
                  else
                  {
                     _loc10_ = int(_loc8_[CharaList.VarTypeMid]);
                     _loc4_ = true;
                     if(!m_callback.GetUserDataValidFlag(ReceiveType_UnlockVarStart + _loc7_))
                     {
                        _loc11_ = int(_loc8_[CharaList.VarTypeUnlockIndex]);
                        _loc12_ = _loc3_;
      ...
      private function waitUnlock(param1:Event) : void
      {
         ...
         while(true)
         {
            ...
            {
               _loc6_ = (_loc6_ = PlayerIndexOwn + _loc2_) % CharacterMax;
      ...
      private function startMain() : void
      {
         ...
         var _loc4_:int;
         if(_loc4_ = m_callback.GetUserDataInt(ReceiveType_FlagLocalBattle))
         {
         ...
         (_loc5_ = getReadyIconMc(0)).visible = true;
         _loc1_ = 0;
         ...
         while(m_chara_list_num > _loc1_)
         {
            ...
            {
               _loc14_ = String(_loc12_[0][CharaList.VarTypeCode]);
               _loc15_ = m_callback.GetUserDataString(ReceiveType_ImageStrStart + _loc1_);
               if(_loc14_ != CharaList.InvalidCode)
               {
                  _loc13_ = new Bitmap(null);
                  IggyFunctions.setTextureForBitmap(_loc13_,_loc15_);
         ...
         while(PlayerNumFri > _loc1_)
         {
            _loc16_ = m_callback.GetUserDataString(ReceiveType_ImageStrNpcStart + _loc1_);
            _loc17_ = new Bitmap(null);
            IggyFunctions.setTextureForBitmap(_loc17_,_loc16_);
         ...
         (_loc6_ = m_timeline.cha_select_cur).icn_lock.visible = false;
         ResetIcons();
         _loc1_ = 0;
         ...
         {
            if(_loc20_ = getMcChamyset(_loc1_))
            {
            }
            _loc1_++;
         }
         var _loc7_:int;
         if((_loc7_ = m_callback.GetUserDataInt(ReceiveType_Time)) <= 0)
         {
      ...
      private function main(param1:Event) : void
      {
         ...
         if(_loc3_ > 0)
         {
            if(m_timer)
            {
               if((_loc10_ = m_timer.GetTime()) <= _loc3_)
               {
         ...
         var _loc6_:MovieClip;
         var _loc5_:MovieClip;
         var _loc7_:MovieClip = (_loc6_ = (_loc5_ = m_timeline.cha_select)["chara_icn_set0" + (m_select_icon_column + 1)])["nest_charaselect0" + (m_select_icon_row + 1)];
         ...
         if(m_flag_decide)
         {
            if(m_timeline.cha_parameter.currentLabel == "start" || m_timeline.cha_parameter.currentLabel == "wait")
            {
               ...
               _loc11_ = int(m_select_info[m_current_player_index][SelectInfoTypeListIndex]);
               if(_loc12_ = getCharaInfo(_loc11_))
               {
                  _loc13_ = _loc12_[m_select_var][CharaList.VarTypeVoiceIdList];
         ...
         if(m_timeline.cha_parameter.currentLabel == "end_comp")
         {
            ...
            var _loc9_:int;
            if(!(_loc9_ = m_callback.GetUserDataInt(ReceiveType_FlagLocalBattle)))
            {
               if(_loc3_ > 0)
               {
      ...
      public function GetSelectVarInfo(param1:int) : Array
      {
         ...
         var _loc2_:int = int(m_select_info[param1][SelectInfoTypeListIndex]);
         var _loc3_:Array = getCharaInfo(_loc2_);
         if(!_loc3_)
         {
            return null;
         }
         var _loc4_:int = (_loc4_ = (_loc4_ = int(m_select_info[param1][SelectInfoTypeVarIndex])) + _loc3_.length) % _loc3_.length;
      ...
      private function getReadyIconMc(param1:int) : MovieClip
      {
         ...
         return m_timeline.cha_parameter.nest_ready["btnact_rb0" + (_loc3_ + 1)];
      }
      ...
      private function changeLR(param1:Boolean) : void
      {
         ...
         do
         {
            ...
            {
               if(!(_loc4_ = m_callback.GetUserDataInt(ReceiveType_FlagSelectAvatar)))
               {
         ...
         do
         {
            if(param1)
            {
               ...
               if(m_select_row < 0)
               {
                  ...
                  --m_select_column;
                  m_select_column += m_chara_num_column;
                  m_select_column %= m_chara_num_column;
               }
            }
            else
      ...
      private function changeVar(param1:Boolean) : void
      {
         ...
         do
         {
            ...
            m_select_var += _loc3_.length;
            m_select_var %= _loc3_.length;
         }
         while(!(_loc4_ = checkUnlockVar(_loc2_,m_select_var)));
      ...
      private function changeCurrentPlayer(param1:Boolean) : void
      {
         ...
         do
         {
            ...
            switch(_loc6_)
            {
               case PlayerTeamTypeFri:
                  ...
                  if((_loc8_ = m_callback.GetUserDataInt(ReceiveType_PartyNpcNum)) > _loc7_)
                  {
                     _loc5_ = false;
                  }
         ...
         {
            _loc10_ = int(m_select_info[m_current_player_index][SelectInfoTypeListIndex]);
      ...
      private function setSelectChara() : void
      {
         ...
         if(_loc4_)
         {
            ...
            while(_loc5_ > _loc1_)
            {
               _loc13_ = (_loc13_ = m_select_var + _loc1_) % _loc5_;
         ...
         var _loc10_:MovieClip = (_loc9_ = (_loc8_ = m_timeline.cha_select)["chara_icn_set0" + (_loc7_ + 1)])["nest_charaselect0" + (_loc6_ + 1)];
         m_select_icon_row = _loc6_;
         m_select_icon_column = _loc7_;
         _loc1_ = 0;
         while(CharaVarIndexNum > _loc1_)
         {
            if(_loc14_ = getMcChamyset(_loc1_))
            {
      ...
      public function sendCharaInfo(param1:int) : void
      {
         ...
         var _loc3_:String = String(_loc2_[CharaList.VarTypeCode]);
         var _loc4_:int = int(m_select_info[param1][SelectInfoTypeVarIndex]);
         var _loc5_:int = int(_loc2_[CharaList.VarTypeMid]);
         var _loc6_:int = int(_loc2_[CharaList.VarTypeModelPreset]);
      ...
      private function updateCharaIcon() : void
      {
         ...
         while(IndexNumColumn + 1 > _loc2_)
         {
            ...
            {
               if(_loc7_ = _loc6_["nest_charaselect0" + (_loc1_ + 1)])
               {
                  ...
                  {
                     _loc7_.visible = true;
                     if(!(_loc9_ = m_callback.GetUserDataInt(ReceiveType_FlagSelectAvatar)) && m_current_player_index != PlayerIndexOwn && checkAvatar(_loc8_))
                     {
                        _loc7_.gotoAndPlay("grayout");
                     }
                     else if(m_select_icon_column == _loc2_ && m_select_icon_row == _loc1_)
                     {
                        (_loc10_ = m_timeline.cha_select_cur).visible = true;
      ...
      private function updateSkill() : void
      {
         ...
         if(m_flag_skill)
         {
            ...
            while(SkillMax > _loc1_)
            {
               ...
               _loc7_ = Number(_loc2_.inact_skill["skill0" + (_loc1_ + 1)].sys_skill.width);
            ...
            _loc4_ = Number(_loc2_.inact_skill.skill08.sys_skill.width);
      ...
      private function checkAvatar(param1:int) : Boolean
      {
         ...
         var _loc3_:String = String(_loc2_[0][CharaList.VarTypeCode]);
